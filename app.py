...
from data import ENTITIES, BOX_CATALOG, PACKAGING_METHODS
...
# --- 탭1: 적재 효율 ---
with tab_calc:
    st.subheader(f"{method} 포장 · {entity_name} 적재 효율")
    if not rows:
    ...
    if not rows:
        ...
        with d1:
            st.download_button(
                ...
                file_name=f'견적요청서_{entity["code"]}_{method}.xlsx',
        ...
        with d2:
            st.download_button(
                ...
                file_name=f'견적요청서_{entity["code"]}_{method}.pdf',
...
with tab_data:
    ...
    st.markdown(f"**{method} · {entity_name}**")
